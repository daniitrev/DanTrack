...
export const ProjectPageStore = signalStore(
  ...
  withComputed((store) => ({
    ...
    currentProject: computed(() => store.project()?.data),
  ...
  withMethods((state) => {
    ...
    return {
      ...
      createProject: rxMethod<CreateProject>(
        pipe(
          exhaustMap((payload) => {
            return loadStateEntity$<ProjectPageResponse | null, CreateProjectResponse | null>({
              patchSlice: (updater) => patchState(state, {page: updater(state.page())}),
              fetch$: () =>
                api.post<CreateProjectResponse | null, CreateProject>('/api/v1/projects', payload),
        ...
        pipe(
          switchMap((payload) => {
            return loadStateEntity$<ProjectDetailResponse | null>({
              ...
              fetch$: () =>
                api.get<ProjectDetailResponse | null>(`/api/v1/projects/${payload.projectId}`),
        ...
        pipe(
          switchMap((payload) => {
            return loadStateEntity$<ProjectDetailResponse | null>({
              patchSlice: (updater) => patchState(state, {project: updater(state.project())}),
              fetch$: () =>
                api.patch<ProjectDetailResponse | null, ProjectUpdate>(
        ...
        pipe(
          switchMap((payload) => {
            return loadStateEntity$<ProjectDetailResponse | null>({
              patchSlice: (updater) => patchState(state, {project: updater(state.project())}),
              fetch$: () =>
                api.delete<ProjectDetailResponse | null>(`/api/v1/projects/${payload.projectId}`),
